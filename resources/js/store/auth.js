...
//   currentAction: null, //現在時刻中のアクションを追跡するためのプロパティ
...
//     context.commit('setCurrentAction', 'register');
...
//     context.commit('setCurrentAction', 'login');
//     context.commit('setApiStatus', null); //apiStatusステートに最初はnullをセットする
//     try { //例外処理
...
//
//     if(context.currentAction === 'register') {
//       context.commit('setRegisterErrorMessages', errors); //登録時のエラー
//     }else if(context.currentAction === 'login') {
//       context.commit('setLoginErrorMessages', errors);    //ログイン時のエラー
//     }
//
//   }else if(error.response && error.response.status === TOO_MANY_REQUEST) {
//     context.commit('setLoginErrorMessages', { general: ['ログイン試行回数が多すぎます。しばらくしてからやり直してください。']});
//
//   }else if(error.response && error.response.data && error.response.data.errors) {
//     context.commit('setLoginErrorMessages', error.response.data.errors); //ログイン時のその他のエラー
//
//   }else {
